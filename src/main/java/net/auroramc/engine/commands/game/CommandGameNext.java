    ...
    public void execute(AuroraMCPlayer player, String aliasUsed, List<String> args) {
        if (args.size() >= 1) {
            if (EngineAPI.getServerState() != ServerState.STARTING_UP && EngineAPI.getServerState() != ServerState.RELOADING_MAPS) {
                ...
                player.getPlayer().sendMessage(AuroraMCAPI.getFormatter().pluginMessage("Game Manager", "You cannot set the next game at this time."));
            }
        } else {
            player.getPlayer().sendMessage(AuroraMCAPI.getFormatter().pluginMessage("Game Manager", "Invalid syntax. Correct syntax: **/game next [game] v[variation] m[map]**"));
        }
    }
